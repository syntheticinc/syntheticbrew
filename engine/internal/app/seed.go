...
- **Know the entities:**
   - An **Agent** needs: name (lowercase letters/digits/hyphens, starts with letter), system_prompt. Optional: model, tools, lifecycle (persistent/ephemeral), tool_execution (sequential/parallel), confirm_before, mcp_servers, max_steps. Delegation targets are NOT an agent field — declare them as agent_relations in a schema (admin_create_agent_relation).
