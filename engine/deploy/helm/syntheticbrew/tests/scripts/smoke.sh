...
kubectl -n "$NAMESPACE" rollout status \
  "deploy/${RELEASE}-syntheticbrew-engine" --timeout=300s
...
if kubectl -n "$NAMESPACE" get \
    "job/${RELEASE}-syntheticbrew-engine-config-apply" --ignore-not-found \
    -o name | grep -q job; then
  kubectl -n "$NAMESPACE" wait --for=condition=complete \
    "job/${RELEASE}-syntheticbrew-engine-config-apply" --timeout=120s

  # Catch v0.4.2 false-positive: brewctl `apply -f <dir>` walks subdirs only,
  # missed top-level ConfigMap-mounted syntheticbrew.yaml → "No changes" → Job
